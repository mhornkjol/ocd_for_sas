...
def compute_ocd_reduced(c0, c1, tau, D, alpha, n, results_dir, boundaries, space="CG", reg="H1"):
    ...
    # phi = Function(Q)
    # z_vector = Expression((0,0,'1'), element=Q.ufl_element())
    # phi.interpolate(z_vector)
    phi = Constant((0,0,1))
    z_val = Function(C, name="Control")
    # expr = Expression('20', element=C.ufl_element())
    # z_val.interpolate(expr)
    ...
    def eval_cb(j, z_val):
        ...
        j0 = j-jr
        values = (j, jr, j0, z_val.vector().max(), numpy.average(z_val.vector()[:]))
    ...
    new_phi = Function(Q, name="Control")
    z_dofs = Q.sub(2).dofmap().dofs()
    new_phi.vector()[z_dofs] = z_val.vector()
...
def map_patient(patient_dir, n, alpha, key, results_dir, normalized, imfolder):
    ...
    if normalized: #! Changed norm to be just from c1
        info("Normalizing")
        c0_norm = assemble(c0*dx)
        c1_norm = assemble(c1*dx)
        c0.vector()[:] = numpy.divide(c0.vector()[:], c1_norm/50000)
...
def run_ocd_reduced(num=None, n=None, beta=None, key=None):
    ...
    dirname = "results_red_ocd_pat%s_n%d_beta%.1e_%s_normalized%s_method_z_vel_scaled_old_form" % (num, n, beta, key, normalized)
    output_dir = os.path.join(patient, "z_dir", dirname)
