...
def compute_ocd_reduced(c0, c1, c_bc, tau, z_height, D, alpha, n, results_dir, boundaries, patient_dir, space="CG", reg="H1"):
    # c0:
    # c1:
    # tau:   time step
    ...
    phi = Function(Q)
    xdmf = XDMFFile(MPI.comm_world, os.path.join(patient_dir, "stokes_solution/velocity.xdmf"))
    ...
    F = (1.0/tau*(c - c_)*d + div(c*z_val*phi)*d)*dx() + diffusion(D, c, d)
    a, L = system(F)
...
def read_image_pair(patient_dir, mesh, key, imfolder):
    ...
    c_bc, z_height = map_image_onto_mesh(i1, mesh, maskfile, is_bc=True)

    # Adjust negative values if any
    c0 = non_negativize(c0)
    c1 = non_negativize(c1)
    c_bc = non_negativize(c_bc)

    info("||c_0||_0 = %g" % norm(c0, "L2"))
    info("||c_1||_0 = %g" % norm(c1, "L2"))
    info("||c_bc||_0 = %g" % norm(c_bc, "L2"))

    return (c0, c1, c_bc, tau, z_height)
...
def map_patient(patient_dir, n, alpha, key, results_dir, use_dti, imfolder):
    ...
    if (not os.path.isfile(meshfile)) or use_dti:
        meshfile = os.path.join(patient_dir, "mesh", "sasflow%d_with_DTI.h5" % n)
    ...
    # Read set of images for this patient
    (c0, c1, c_bc, tau, z_height) = read_image_pair(patient_dir, mesh, key, imfolder)
    ...
    space = "CG"
    reg = "H1"
    (c, phi) = compute_ocd_reduced(c0, c1, c_bc, tau, z_height, D, alpha, n, results_dir, boundaries, patient_dir,
                                   space=space, reg=reg)
    ...
    if "with_DTI" in meshfile:
        shutil.copy(meshfile, name("sasflow%d_with_DTI.h5" % n))
    else:
...
def run_ocd_reduced(num=None, n=None, beta=None, key=None):
    ...
    dirname = "results_red_ocd_pat%s_n%d_beta%.1e_%s_data" % (num, n, beta, key)
    output_dir = os.path.join(patient, "stokes", dirname)
