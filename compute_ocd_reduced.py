...
def compute_ocd_reduced(c0, c1, c_bc, tau, z_height, D, alpha, n, results_dir, boundaries, space="CG", reg="H1"):
    ...
    F = (1.0/tau*(c - c_)*d - inner(c*phi,grad(d)))*dx() + diffusion(D, c, d)
    a, L = system(F)
    ...
    if not os.path.isdir(dirname):
        os.mkdir(dirname)
    header = ("j", "jr", "j0", "\max \phi", "avg \phi")
    ...
    def eval_cb(j, phi):
        ...
        values = (j, jr, j0, phi.vector().max(), avg_phi)
    ...
    h = Function(Q)
    h.vector()[:] = bar(phi, omega)
    taylor_test(Jhat, phi, h)
...
def run_ocd_reduced(num=None, n=None, beta=None, key=None):
    ...
    output_dir = os.path.join(patient, "testing", dirname)
