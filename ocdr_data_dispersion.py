...
def compute_ocd_reduced(c0, c1, c_bc, tau, z_height, D, alpha, n, results_dir, boundaries, space="CG", reg="H1"):
    ...
    d = TestFunction(C)
    timesteps = 10
    times = numpy.linspace(0, tau, timesteps)
...
def map_patient(patient_dir, n, alpha, key, results_dir, use_dti, imfolder, dispersion):
    """
...
def run_ocd_reduced(num=None, n=None, beta=None, key=None):
    ...
    dirname = "results_red_ocd_pat%s_n%d_beta%.1e_k_%s_dispersion_%s" % (num, n, beta, key, dispersion)
    output_dir = os.path.join(patient, dirname)
    info("Setting %s as output directory" % output_dir)

    # Create OCD(R) map
    (c, phi) = map_patient(patient, n, beta, key, output_dir, use_dti, imfolder, dispersion)
