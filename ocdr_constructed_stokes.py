...
def compute_ocd_reduced(c0, c1, tau, D, alpha, n, results_dir, boundaries, patient_dir, space="CG", reg="H1"):
    # c0:
    # c1:
    # tau:   time step
    ...
    phi = Function(Q)
    xdmf = XDMFFile(MPI.comm_world, os.path.join(patient_dir, "stokes_vel_fields/stokes_vel_field_%s/velocity.xdmf" % n))
    xdmf.read_checkpoint(phi, "velocity", 0)

    z_val = Constant(0.0, name="Control")

    # Regularization term
    def R(z_val, phi, alpha, mesh):
        if reg == "H1":
            form = 0.5*alpha*(inner(grad(z_val*phi), grad(z_val*phi)))*dx(domain=mesh) # 0.5*alpha*(inner(phi, phi) + inner(grad(phi), grad(phi)))*dx(domain=mesh)
        else:
            form = 0.5*alpha*(inner(div(z_val*phi), div(z_val*phi)))*dx(domain=mesh) # 0.5*alpha*(inner(phi, phi) + inner(div(phi), div(phi)))*dx(domain=mesh)
        return form
    ...
    F = (1.0/tau*(c - c_)*d - inner(c*z_val*phi,grad(d)))*dx() + diffusion(D, c, d)
    a, L = system(F)
    ...
    j = 0.5*(c - c2)**2*dx(domain=mesh) + R(z_val, phi, alpha, mesh)
    J = assemble(j)
    info("J (initial) = %f" % J)
    ...
    if not os.path.isdir(dirname):
        os.mkdir(dirname)
    header = ("j", "jr", "j0", "\max z_val", "avg z_val")

    def eval_cb(j, z_val):
        jr = assemble(R(z_val, phi, alpha, mesh))
        j0 = j-jr
        values = (j, jr, j0, z_val.values()[0], z_val.values()[0])
    ...
    # Minimize functional
    tol = 1.0e-8
    z_val_opt = minimize(Jhat,
                       tol=tol, 
                       options={"gtol": tol, "maxiter": 200, "disp": True})
    pause_annotation()

    # Update phi, and do a final solve to compute c
    z_val.assign(z_val_opt)
    solve(a == L, c, bc, solver_parameters={"linear_solver": "mumps"})
    print("z_val value: ", z_val.values()[0])
    new_phi = project(z_val*phi, Q, solver_type='mumps')

    J = assemble(j)
    j0 = 0.5*(c - c2)**2*dx(domain=mesh)
    jr = R(z_val, phi, alpha, mesh)
    J0 = assemble(j0)
    Jr = assemble(jr)
    info("J  = %f" % J)
    info("J0 = %f" % J0)
...
def map_patient(patient_dir, n, alpha, key, results_dir, normalized, imfolder):
    """
    ...
    space = "CG"
    reg = "H1"
    (c, phi) = compute_ocd_reduced(c0, c1, tau, D, alpha, n, results_dir, boundaries, patient_dir,
                                   space=space, reg=reg)
...
def run_ocd_reduced(num=None, n=None, beta=None, key=None):
    ...
    dirname = "results_red_ocd_pat%s_n%d_beta%.1e_%s_normalized%s_constructed" % (num, n, beta, key, normalized)
    output_dir = os.path.join(patient, "investigate_stokes_vel", dirname)
    info("Setting %s as output directory" % output_dir)

    # Create OCD(R) map
    (c, phi) = map_patient(patient, n, beta, key, output_dir, normalized, imfolder)
